...
def losmoments2skewt(w: Callable, v_c: Callable, alpha: Callable, nu: Callable):
    def pdf_los(vlos, r_perp, r_parallel):
        ...
        return skewt.pdf(
            ...
            v_c=v_c(sorted_r_perp, sorted_r_parallel)[idx_to_unsort_perp,:][:,idx_to_unsort_parallel],
            alpha=alpha(sorted_r_perp, sorted_r_parallel)[idx_to_unsort_perp,:][:,idx_to_unsort_parallel],
            nu=nu(sorted_r_perp, sorted_r_parallel)[idx_to_unsort_perp,:][:,idx_to_unsort_parallel],
        )
